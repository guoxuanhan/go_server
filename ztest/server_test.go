...
/*
	服务器单元测试
	go test -v ./ztest -run=TestServer
*/
...
func (this *HelloRouter) Handle(request ziface.IRequest) {
	...
	err := request.GetConn().SendMsg(2, []byte("hello zix hello Router"))
	if err != nil {
		fmt.Println(err)
	}
}

func DoConnectionBegin(conn ziface.IConn) {
...
//连接断开的时候执行
func DoConnectionLost(conn ziface.IConn) {
