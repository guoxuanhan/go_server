...
type TimeWheel struct {
	...
	*/
	...
	//互斥锁（继承RWMutex的 RWLock,UnLock等方法）
	sync.RWMutex
}
...
//删除一个定时器（根据定时器的唯一id标识）
func (tw *TimeWheel) RemoverTimer(tid uint32) {
	tw.Lock()
	defer tw.Unlock()
...
func (tw *TimeWheel) run() {
	for {
		...
		//当前定时器要重新添加 所以给当前刻度再重新开辟一个map Timer容器
		...
		//取出下一个刻度挂载的全部定时器，进行重新添加（为了安全起见，待考虑）
...
func (tw *TimeWheel) Run() {
	go tw.run()
	zlog.Info("TimeWheel name = ", tw.name, " is running...")
}
